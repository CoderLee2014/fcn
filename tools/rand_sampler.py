...
class RandSampler(object):
    ...
    def sample(self, label):
        ...
        Parameters:
        ----------
        label : numpy.array (n x 9 matrix)
            ground-truths
...
class RandCropper(RandSampler):
    ...
    min_overlap : float
        threshold of minimum overlap between a rand crop and any gt
    max_trials : int
        maximum trials, if exceed this number, give up anyway
    max_sample : int
        maximum random crop samples to be generated
        ...
        for trial in range(self.max_trials):
            ...
            if ious is not None:
                ...
                for i in range(valid_mask.size):
                    if ious[i] > 0:
                        x1 = max(0., (gt[i, 1] - l)) / new_width
                        y1 = max(0., (gt[i, 2] - t)) / new_height
                        x2 = min(1., (gt[i, 3] - l)) / new_width
                        y2 = min(1., (gt[i, 4] - t)) / new_height
                        x3 = min(1., (gt[i, 5] - l)) / new_width
                        y3 = min(1., (gt[i, 6] - t)) / new_height
                        x4 = max(0., (gt[i, 7] - l)) / new_width
                        y4 = max(0., (gt[i, 8] - t)) / new_height
                        new_gt_boxes.append([gt[i, 0], x1, y2, x2, y2, x3, y3, x4, y4])
                if not new_gt_boxes:
                    continue
