...
def load_icdar(image_set, year, dirname, shuffle=False):
    """
    ...
    Imdb
    """ 
    Imdb = ICDAR(image_set, year, dirname, shuffle, is_train=True)
    return Imdb           
...
def parse_args():
    ...
    parser.add_argument('--target', dest='target', help='output list file',
                        default=os.path.join(curr_path, '../data/ICDAR', 'val.lst'),
                        type=str)
    parser.add_argument('--root', dest='root_path', help='dataset root path',
                        default=os.path.join(curr_path, '..', 'data'),
                        type=str)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'icdar':
        db = load_icdar("icdar2015_val", "2015", args.root_path, args.shuffle)
    ...
    subprocess.check_call(["python",
        ...
        "--shuffle", str(int(args.shuffle)), "--pack-label", "0"])
